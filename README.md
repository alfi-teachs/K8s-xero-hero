## 📚 Course Roadmap

### Level 1 - Kubernetes Basics


# 🚀 Kubernetes Xero to Hero

A complete hands-on Kubernetes learning repository designed for beginners who want to become confident Kubernetes engineers.

---

## 📚 Course Roadmap

### Level 1 - Kubernetes Basics

| Lab | Topic | Status |
|------|-------|--------|
| 01 | [Install Kubernetes](labs/lab-01-install-kubernetes.md) | ⬜ |
| 02 | [First Kubernetes Pod](labs/lab-02-first-pod.md) | ⬜ |
| 03 | [Multi-Container Pod](labs/lab-03-multi-container-pod.md) | ⬜ |
| 04 | [Labels and Selectors](labs/lab-04-labels-selectors.md) | ⬜ |

---

### Level 2 - Controllers

| Lab | Topic | Status |
|------|-------|--------|
| 05 | [ReplicaSet](labs/lab-05-replicaset.md) | ⬜ |
| 06 | [Deployment](labs/lab-06-deployment.md) | ⬜ |
| 07 | [Rolling Update](labs/lab-07-rolling-update.md) | ⬜ |
| 08 | [Rollback](labs/lab-08-rollback.md) | ⬜ |

---

### Level 3 - Networking

| Lab | Topic | Status |
|------|-------|--------|
| 09 | [ClusterIP Service](labs/lab-09-clusterip-service.md) | ⬜ |
| 10 | [NodePort Service](labs/lab-10-nodeport-service.md) | ⬜ |
| 11 | [LoadBalancer Service](labs/lab-11-loadbalancer.md) | ⬜ |
| 12 | [Ingress](labs/lab-12-ingress.md) | ⬜ |

---

### Level 4 - Configuration

| Lab | Topic | Status |
|------|-------|--------|
| 13 | [ConfigMap](labs/lab-13-configmap.md) | ⬜ |
| 14 | [Secret](labs/lab-14-secret.md) | ⬜ |

---

### Level 5 - Storage

| Lab | Topic | Status |
|------|-------|--------|
| 15 | [EmptyDir](labs/lab-15-emptydir.md) | ⬜ |
| 16 | [Persistent Volume (PV)](labs/lab-16-persistent-volume.md) | ⬜ |
| 17 | [HostPath Volume](labs/lab-17-hostpath.md) | ⬜ |
| 18 | [Taints and Tolerations](labs/lab-18-taints-tolerations.md) | ⬜ |

---

### Level 6 - Scheduling

| Lab | Topic | Status |
|------|-------|--------|
| 19 | [Node Selector](labs/lab-19-node-selector.md) | ⬜ |
| 20 | [Node Affinity](labs/lab-20-node-affinity.md) | ⬜ |                  
| 21 | [CPU Requests](labs/lab-21-cpu-memory-requests.md) | ⬜ |  

---

### Level 7 - Resource Management

| Lab | Topic | Status |
|------|-------|--------|
| 22 | [CPU Limits](labs/lab-22-cpu-memory-limits.md) | ⬜ |
| 23 | [Namespaces](labs/lab-23-namespace.md) | ⬜ |
| 24 | 

---

### Level 8 - Autoscaling

| Lab | Topic | Status |
|------|-------|--------|
| 25 | [Metrics Server](labs/lab-25-metrics-server.md) | ⬜ |
| 26 | [Horizontal Pod Autoscaler](labs/lab-26-hpa.md) | ⬜ |

---

### Level 9 - Security

| Lab | Topic | Status |
|------|-------|--------|
| 27 | [ServiceAccount](labs/lab-27-serviceaccount.md) | ⬜ |
| 28 | [Role and RoleBinding](labs/lab-28-role-rolebinding.md) | ⬜ |
| 29 | [ClusterRole and ClusterRoleBinding](labs/lab-29-clusterrole.md) | ⬜ |

---

### Level 10 - Jobs

| Lab | Topic | Status |
|------|-------|--------|
| 30 | [Job](labs/lab-30-job.md) | ⬜ |
| 31 | [CronJob](labs/lab-31-cronjob.md) | ⬜ |

---

### Level 11 - Troubleshooting

| Lab | Topic | Status |
|------|-------|--------|
| 32 | [CrashLoopBackOff](labs/lab-32-crashloopbackoff.md) | ⬜ |
| 33 | [ImagePullBackOff](labs/lab-33-imagepullbackoff.md) | ⬜ |
| 34 | [Pending Pods](labs/lab-34-pending-pods.md) | ⬜ |
| 35 | [Service Troubleshooting](labs/lab-35-service-troubleshooting.md) | ⬜ |
| 36 | [DNS Troubleshooting](labs/lab-36-dns-troubleshooting.md) | ⬜ |

---

### Level 12 - Monitoring

| Lab | Topic | Status |
|------|-------|--------|
| 37 | [Prometheus](labs/lab-37-prometheus.md) | ⬜ |
| 38 | [Grafana](labs/lab-38-grafana.md) | ⬜ |

---

### Level 13 - Projects

| Lab | Topic | Status |
|------|-------|--------|
| 39 | [Three-Tier Application](labs/lab-39-three-tier-app.md) | ⬜ |
| 40 | [Production Kubernetes Project](labs/lab-40-production-project.md) | ⬜ |

---

